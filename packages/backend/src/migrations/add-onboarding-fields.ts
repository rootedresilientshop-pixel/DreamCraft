 ...
 *
 * This migration adds the following fields to all existing users:
 * - profile.profileCompleted (boolean)
 * - profile.primarySkill (string, optional)
 * - onboarding object with tracking fields
 *
 * For existing collaborators with skills, the first skill is set as primary.
 * All existing users are marked as having completed onboarding (grandfathered in).
 */
...
interface UserDocument {
  ...
  onboarding?: {
    collaboratorWizardCompleted?: boolean;
    creatorIntroShown?: boolean;
    completedAt?: Date;
  };
  [key: string]: any;
}
